...
let test_izenpe () =
  ...
  let expected = "2.5.4.9=#417664612064656c204d65646974657272616e656f2045746f726269646561203134202d203031303130205669746f7269612d4761737465697a,O=IZENPE S.A. - CIF A01337260-RMerc.Vitoria-Gasteiz T1055 F62 S8" in
...
let test_distinguished_name_pp () =
  ...
  let pp1 = "RFC_comma", Fmt.hbox (Dn.make_pp ~style:`RFC_comma ()) in
  let pp2 = "RFC_comma_sp", Fmt.hbox (Dn.make_pp ~style:`RFC_comma_sp ()) in
  let pp3 = "RFC_semi", Fmt.hbox (Dn.make_pp ~style:`RFC_semi ()) in
  let pp4 = "RFC_semi_sp", Fmt.hbox (Dn.make_pp ~style:`RFC_semi_sp ()) in
  let pp5 = "OSF", Fmt.hbox (Dn.make_pp ~style:`OSF ()) in
  let pp6 = "RFC_comma, vbox", Fmt.vbox (Dn.make_pp ~style:`RFC_comma ()) in
  ...
  check pp3 dn1 {|CN=John Doe+Initials=J.D.+Initials=N.N.;O=Blanc|} ;
  check pp3 dn2 {|CN=\ \"\+\,\;/\<\>\\ \ ;CN=\# 2;O=Escapist|} ;
  check pp4 dn1 {|CN=John Doe+Initials=J.D.+Initials=N.N.; O=Blanc|} ;
  check pp4 dn2 {|CN=\ \"\+\,\;/\<\>\\ \ ; CN=\# 2; O=Escapist|} ;
  check pp5 dn1 {|/O=Blanc/CN=John Doe+Initials=J.D.+Initials=N.N.|} ;
  check pp5 dn2 {|/O=Escapist/CN=\# 2/CN=\ \"\+,;\/\<\>\\ \ |} ;
  check pp6 dn1 "CN=John Doe+Initials=J.D.+Initials=N.N.,\nO=Blanc"
