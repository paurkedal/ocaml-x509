...
let test_distinguished_name_pp () =
  ...
  let pp1 = "RFC4514",
    Fmt.hbox (Dn.make_pp ~preset:`RFC4514 ()) in
  let pp2 = "RFC4514-spacy",
    let rdn_sep = Fmt.semi in
    let ava_sep = Fmt.(any " +" ++ sp) in
    let ava_equal = Fmt.(any " = ") in
    Fmt.hbox (Dn.make_pp ~preset:`RFC4514 ~rdn_sep ~ava_sep ~ava_equal ()) in
  let pp3 = "OpenSSL", Fmt.hbox (Dn.make_pp ~preset:`OpenSSL ()) in
  let pp4 = "OSF", Fmt.hbox (Dn.make_pp ~preset:`OSF ()) in
  let pp5 = "RFC4514-vbox", Fmt.vbox (Dn.make_pp ~preset:`RFC4514 ()) in
